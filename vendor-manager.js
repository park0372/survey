  ...
  function refreshDashboard(){
      ...
      if(!surveyChart){
        surveyChart = new Chart(canvas.getContext('2d'), {
          ...
          data:{
            ...
            datasets:[{data:[approved,review,pending,survey], borderWidth:0, radius:'75%', cutout:'60%'}]
          },
    ...
    if(recent){
      ...
      data.slice().reverse().slice(0,6).forEach(v => {
        ...
        card.innerHTML='<h4>'+esc(v.name || '')+'</h4><p>'+esc(v.category || '')+'</p><span>'+esc(v.city || v.location || '')+'</span>';
