...
<button class="table-action view-btn" onclick="showVendorDetail(${index})" title="View vendor">View</button>
<button class="table-action edit-btn" onclick="editVendor(${index})" title="Edit vendor">Edit</button>
<button class="table-action delete-btn" onclick="deleteVendor(${index})" title="Delete vendor">Delete</button>
...
document.getElementById("detailRemarks").value = v.remarks || "";
    ...
    // Reset the browser file inputs whenever the selected Vendor changes.
    // The native file input must never carry another Vendor's selected file.
    ...
    if (isoCertificateInput) isoCertificateInput.value = "";
    if (companyRegistrationInput) companyRegistrationInput.value = "";

    const docsForVendor = v.documents || {};
    ...
    if (companyProfileName) {
        companyProfileName.textContent =
            docsForVendor.companyProfile || "선택된 파일 없음";
    }

    if (isoCertificateName) {
        isoCertificateName.textContent =
            docsForVendor.isoCertificate || "선택된 파일 없음";
    }

    if (companyRegistrationName) {
        companyRegistrationName.textContent =
            docsForVendor.companyRegistration || "선택된 파일 없음";
    }
...
// ============================================
// Previous / Next Vendor
// ============================================
function updateVendorNavigation() {
    const prevBtn = document.getElementById("prevVendorBtn");
    const nextBtn = document.getElementById("nextVendorBtn");

    if (prevBtn) {
        prevBtn.disabled = selectedVendorIndex === null || selectedVendorIndex <= 0;
    }
    if (nextBtn) {
        nextBtn.disabled = selectedVendorIndex === null || selectedVendorIndex >= vendors.length - 1;
    }
}

function showPreviousVendor() {
    if (selectedVendorIndex === null || selectedVendorIndex <= 0) return;
    showVendorDetail(selectedVendorIndex - 1);
}

function showNextVendor() {
    if (selectedVendorIndex === null || selectedVendorIndex >= vendors.length - 1) return;
    showVendorDetail(selectedVendorIndex + 1);
}

const prevVendorBtn = document.getElementById("prevVendorBtn");
const nextVendorBtn = document.getElementById("nextVendorBtn");
if (prevVendorBtn) prevVendorBtn.addEventListener("click", showPreviousVendor);
if (nextVendorBtn) nextVendorBtn.addEventListener("click", showNextVendor);
...
function handleDocumentFile(inputId, documentKey, nameElementId) {
    ...
    input.addEventListener("change", function () {
        ...
        // Capture the file name, then clear the native file input.
        // The persistent filename is displayed in the vendor-specific label below.
