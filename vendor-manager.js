...
function ensureVendorModal() {
    ...
    m.innerHTML = `
      <div class="modal-content">
        <h2>Add Vendor</h2>
        <input id="vendorCompany" placeholder="Company Name">
        <select id="vendorCategory">
          <option value="">Select Category</option>
          <option>Civil</option><option>Electrical</option><option>Mechanical</option>
          <option>Material</option><option>Housing</option>
        </select>
        <input id="vendorCity" placeholder="City">
        <input id="vendorContact" placeholder="Contact Person">
        <input id="vendorPhone" placeholder="Phone">
        <input id="vendorEmail" placeholder="Email">
        <input id="vendorWebsite" placeholder="Website">
        <select id="vendorStatus">
          <option>Approved</option><option>Pending</option><option>Review</option>
        </select>
        <div class="modal-buttons">
          <button id="saveVendor" class="primary-btn" type="button">Save</button>
...
if (vendorAddBtn) vendorAddBtn.addEventListener("click", () => {
...
if (detailVendorId) detailVendorId.value = v.id || "";
...
const statusDiv = document.getElementById("documentStatus");
    ...
    (docs.companyRegistration ? 1 : 0);
        if (attachmentCount) attachmentCount.textContent = `${uploadedCount} / 3`;
...
if (saveVendorBtn) saveVendorBtn.addEventListener("click", saveVendorDetail);
...
function saveDocuments() {
    ...
    URL.createObjectURL(companyProfile);
        ...
        if (companyProfileName) companyProfileName.textContent = companyProfile.name;
    }
    // ISO Certificate
...
vendors[selectedVendorIndex].documents.isoCertificateURL =
    ...
    if (isoCertificateName) isoCertificateName.textContent = isoCertificate.name;
}
...
vendors[selectedVendorIndex].documents.companyRegistrationURL =
    ...
    if (companyRegistrationName) companyRegistrationName.textContent = companyRegistration.name;
}
vendors[selectedVendorIndex].lastUpdate =
    new Date().toISOString().split("T")[0];
    // LocalStorage 저장
...
const companyProfileFile = document.getElementById("companyProfileFile");
if (companyProfileFile) companyProfileFile.addEventListener("change", function () {
