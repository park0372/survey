...
(function () {
  ...
  function updateKPI() {
    ...
    // Other common KPI IDs, when present.
    const approved = document.getElementById("approvedVendor");
    if (approved) {
      approved.textContent = String(
        vendors.filter(v => String(v.status || v.approval || "").toLowerCase() === "approved").length
      );
    }
    ...
    if (pending) {
      pending.textContent = String(
        vendors.filter(v => String(v.status || v.approval || "").toLowerCase() === "pending").length
      );
    }
  }
