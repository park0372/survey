...
function normalizeVendorDates() {
    vendors.forEach(v => { if (v) v.lastUpdate = formatVendorDate(v.lastUpdate); });
}

// Remove accidental blank/invalid vendor records from localStorage.
// This does NOT change valid vendor data or Add Vendor behavior.
...
const companySearch = document.getElementById("vendorSearch");
...
<td>${formatVendorDate(v.lastUpdate)}</td>
...
/* Save */
if (saveVendor) {
    saveVendor.addEventListener("click", function () {
...
const isoCertificateFile = document.getElementById("isoCertificateFile");

if (isoCertificateFile) {
    isoCertificateFile.addEventListener("change", function () {
