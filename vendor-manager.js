...
/* ===========================
   IVIP Dashboard Live Sync
   Survey Progress + Recent Companies
   Uses the same vendor data managed above.
=========================== */
(function(){
  function dashboardVendors(){
    try {
      const saved = localStorage.getItem('ivip_vendors');
  ...
  function refreshDashboard(){
    const data = dashboardVendors();
    const approved = data.filter(v => String(v.status || v.approval || '').toLowerCase() === 'approved').length;
    const pending = data.filter(v => String(v.status || v.approval || '').toLowerCase() === 'pending').length;
    const review = data.filter(v => String(v.status || v.approval || '').toLowerCase() === 'review').length;
    const survey = data.filter(v => String(v.status || v.approval || '').toLowerCase() === 'survey').length;
    ...
    if(percentEl) percentEl.textContent = data.length ? Math.round(approved / data.length * 100) + '%' : '0%';

    /* Add the two reference-design status rows without changing vendor data. */
    const summary = document.querySelector('.survey-summary');
    if(summary){
      let reviewRow = document.getElementById('surveyReviewRow');
      if(!reviewRow){
        reviewRow = document.createElement('div');
        reviewRow.id = 'surveyReviewRow';
        reviewRow.className = 'survey-stat review-stat';
        reviewRow.innerHTML = '<span>Review</span><b id="surveyReview">0</b>';
        const pendingRow = summary.querySelector('.pending-stat');
        summary.insertBefore(reviewRow, pendingRow || null);
      }
      let surveyRow = document.getElementById('surveySurveyRow');
      if(!surveyRow){
        surveyRow = document.createElement('div');
        surveyRow.id = 'surveySurveyRow';
        surveyRow.className = 'survey-stat survey-status-stat';
        surveyRow.innerHTML = '<span>Survey</span><b id="surveySurvey">0</b>';
        const totalRow = summary.querySelector('.total-stat');
        summary.insertBefore(surveyRow, totalRow || null);
      }
      const reviewEl = document.getElementById('surveyReview');
      const surveyEl = document.getElementById('surveySurvey');
    ...
    if(canvas && typeof Chart !== 'undefined'){
      if(!surveyChart){
        surveyChart = new Chart(canvas.getContext('2d'), {
          type:'doughnut',
          data:{
            labels:['Approved','Review','Pending','Survey'],
            datasets:[{
              data:[approved,review,pending,survey],
              borderWidth:0,
              radius:'78%',
              cutout:'62%'
            }]
        ...
        surveyChart.data.datasets[0].data = [approved,review,pending,survey];
        surveyChart.update();
      }
    }

    const recent = document.getElementById('recentCompanies');
    if(recent){
      recent.innerHTML = '';
      data.slice().reverse().slice(0,6).forEach(v => {
        const card = document.createElement('div');
        card.className = 'recent-card';

        const status = String(v.status || v.approval || 'Approved');
        const date = v.lastUpdate || v.updatedAt || new Date().toISOString().slice(0,10);
        card.innerHTML =
          '<h4>'+esc(v.name || '')+'</h4>' +
          '<p>'+esc(status)+'</p>' +
          '<span>'+esc(date)+'</span>';
  ...
  window.addEventListener('ivip:vendorsUpdated', refreshDashboard);
